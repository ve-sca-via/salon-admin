...
import { useGetSystemConfigsQuery, useUpdateSystemConfigMutation, useCreateSystemConfigMutation, useDeleteSystemConfigMutation, useGetAvailableSystemConfigKeysQuery } from '../services/api/configApi';

export const SystemConfig = () => {
  const { data: configsData, isLoading, error } = useGetSystemConfigsQuery();
  const { data: availableKeysData } = useGetAvailableSystemConfigKeysQuery();
  ...
  const availableKeys = availableKeysData || [];
  const unconfiguredKeys = availableKeys.filter(ak => !configs.some(c => c.config_key === ak.config_key));
        ...
        <>
          ...
          <Card>
            <div className="p-12 text-center">
              ...
              <p className="text-gray-600 mb-4">
                The system_config table is empty. Select a configuration from the predefined list to get started.
              </p>
            </div>
          </Card>
        </>
                        ...
                        <div className="space-y-4 mt-4">
                          <div>
                            ...
                            <div className="relative">
                              <input
                                type={isSensitiveConfig(config.config_key) && !showValue[`edit_${config.id}`] ? "password" : "text"}
                              ...
                              {isSensitiveConfig(config.config_key) && (
                                <button
                                  type="button"
                                  onClick={() => setShowValue(prev => ({ ...prev, [`edit_${config.id}`]: !prev[`edit_${config.id}`] }))}
                                  className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-500 hover:text-gray-700"
                                >
                                  {showValue[`edit_${config.id}`] ? (
      ...
      >
        <form onSubmit={handleCreate} className="space-y-5">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                const matchedConfig = availableKeys.find(ak => ak.config_key === selectedKey);
                setNewConfig({ 
                  ...newConfig, 
                  config_key: selectedKey,
                  description: matchedConfig ? matchedConfig.description : newConfig.description,
                  config_type: matchedConfig ? matchedConfig.config_type : newConfig.config_type
                });
              }}
              ...
              required
            >
              <option value="" disabled>Select a configuration key...</option>
              {unconfiguredKeys.map((ak) => (
                <option key={ak.config_key} value={ak.config_key}>
                  {ak.label} ({ak.config_key})
                </option>
              ))}
            </select>
            {unconfiguredKeys.length === 0 && (
              <p className="text-sm text-amber-600 mt-2">All predefined platform configurations have been added.</p>
            )}
          </div>
          ...
          <div>
            ...
            <div className="relative">
              <input
                type={isSensitiveConfig(newConfig.config_key) && !showValue['new'] ? "password" : "text"}
                ...
                required
              />
              {isSensitiveConfig(newConfig.config_key) && (
                <button
                  type="button"
